...
const AddStudents = ({ onClose, onSwitchToSignIn }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      role_id: '3',//role Id set to Student
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white rounded-xl p-4 md:p-8 w-full max-w-md border border-gray-300">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-black">Enroll A Student</h2>

        </div>
        {isAlreadyRegistered ? (
          <p className="text-red-500">User already registered with this email.</p>
        ) : isSignupSuccessful ? (
          <p className="text-red-500">Student Enrolled successful!</p>
        ) : (
